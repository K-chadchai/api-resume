...
export class UsersService extends TypeOrmCrudService<UsersEntity> {
  ...
  async postImage(req, res, query) {
    ...
    const callback = async (runner: QueryRunner, result) => {
      ...
      const imageOrigin: ImagesEntity[] = files.filter(
        (item: ImagesEntity) => item.suffix === 'x',
      );
      if (!imageOrigin || imageOrigin.length == 0)
        this.throwBadRequestException(
          `Not found original image, images.suffix='x'`,
        );
    ...
    return this.mediasService.uploadMedia(
      req,
      res,
  ...
  async getImage(employee_id) {
    ...
    if (!user)
      this.throwBadRequestException(
        `Not found users.employee_id=${employee_id}`,
      );
    ...
    if (!image)
      this.throwBadRequestException('Not found images.s3key=' + s3key);

    // mediaId
    const mediaId = image.mediaId;
    if (!mediaId)
      this.throwBadRequestException(`Not found mediaId,images.s3key=${s3key}`);
    ...
    if (!media.mimetype)
      this.throwBadRequestException(`Not found mimetype,medias.id=${mediaId}`);
