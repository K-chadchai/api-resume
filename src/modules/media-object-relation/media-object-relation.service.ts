...
export class MediaObjectRelationService extends TypeOrmCrudService<MediaObjectRelationEntity> {
  constructor(@InjectRepository(MediaObjectRelationEntity) repo, private readonly appService: AppService) {
    super(repo);
  }
  ...
  async getArticleDepartUnitSide(props: IGetByArticleDepartUnitSide) {
    ...
    const query = await getConnection()
      ...
      .leftJoinAndSelect(MediaSideEntity, 'media_side', 'media_side.id = media_object_relation.article_side_id')
      .leftJoinAndSelect(
      ...
      .orWhere(side ? `media_side.id in (${side})` : `media_side.side_name = ''`)
      .orWhere(depart ? `media_depart.id in (${depart})` : `media_depart.code = ''`)
      .orWhere(
        props.last_edited
